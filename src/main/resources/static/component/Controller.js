...
function refresh(){
    var points = axios.get('http://localhost:8080/point').then(points => {
        clear();
        if (points.data != null) {
...
function restart() {
    clear();
    axios.post('http://localhost:8080/restart');
}

function registro() {
    var x = document.getElementById("inputText").value;
    axios.get('http://localhost:8080/setname?name=' + x);
}